...
<div style="width:50%;float:left;text-align:right;"><b>Цена след ремонт</b>: {{$repair->price_after ?: $repair->price}}лв. / {{ $repair->price_after > $repair->price ? $repair->price_after * getCurrencyRate('EUR') : $repair->price * getCurrencyRate('EUR') }}€</div>
<hr style="clear:both;"/>
@endif

<div style="float: left; width: 33.33%;"><b>Цена: </b>{{$repair->price_after > $repair->price ? $repair->price_after : $repair->price}}лв. / {{$repair->price_after > $repair->price ? $repair->price_after * getCurrencyRate('EUR') : $repair->price * getCurrencyRate('EUR')}}€</div>
<div style="float: left; width: 33.33%;text-align:center;"><b>Капаро: </b>{{$repair->prepaid}}лв. / {{$repair->prepaid * getCurrencyRate('EUR')}}€</div>
<div style="float: left; width: 33.33%;text-align:right;"><b>Остатък: </b>{{$repair->price - $repair->prepaid}} лв. / {{ ($repair->price - $repair->prepaid) * getCurrencyRate('EUR') }}€</div>
<hr style="clear:both;"/>
