<tr data-id="{{$repair->id}}">
    ...
    <td> @if($repair->status == 'done') <span class="return-repair-action" data-url="repairs/return/{{$repair->barcode}}" data-repair-return><i class="c-brown-500 ti-reload"></i></span> @endif
        @if($repair->status == 'repairing' || $repair->status !== 'done' || $repair->status == 'returning') <span data-url="repairs/{{$repair->barcode}}" class="edit-btn" data-form-type="edit" data-form="repairs" data-toggle="modal" data-target="#fullEditRepair"><i class="c-brown-500 ti-pencil"></i></span> @endif
