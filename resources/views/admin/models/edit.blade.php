      ...
      <div class="form-row">
        ...
        <div class="form-group col-md-6">
          ...
          <select id="jewel_edit" name="jewel_id" class="form-control calculate">
            ...
            <option value="{{ $jewel->id }}" data-material="{{ $jewel->material_id }}" @if($model->jewel_id == $jewel->id)
              selected @endif>{{ $jewel->name }}
            </option>
            @endforeach
          </select>
        </div>
            ...
            <select id="material_type" name="material_id[]" class="material_type form-control calculate"
             data-calculatePrice-material>
              ...
              <option value="{{ $material->id }}" data-material="{{ $material->id }}" data-pricebuy="{{ $material->material->pricesBuy->first()->price }}"
               @if($material->id == $option->material_id) selected @endif>{{ $material->material->parent->name }} - {{
                $material->material->color }} - {{ $material->material->carat }}</option>
              @endif
              @endforeach
            </select>
          </div>
            ...
            <select id="retail_price_edit" name="retail_price_id[]" class="form-control  calculate prices-filled retail-price"
             data-calculatePrice-retail>
              ...
              <option value="{{ $price->id }}" data-material="{{ $price->material_id }}" data-price="{{ $price->price }}"
               @if($option->retail_price_id == $price->id) selected @endif>{{ $price->slug }} - {{ $price->price }}</option>
              @endforeach
            </select>
          </div>
      ...
      <div class="form-row">
        <div class="form-group col-md-6 weight-holder">
          ...
          <div class="input-group">
            <input type="number" class="form-control  calculate" id="weight_edit" value="{{ $model->weight }}" name="weight"
        ...
        <div class="form-row fields">
          <div class="form-group col-md-6">
            ...
            <select id="model-stone" name="stones[]" class="form-control" data-calculatePrice-stone>
              ...
              <option value="{{ $stone->id }}" @if($modelStone->stone->id == $stone->id) selected @endif data-stone-type="{{
          ...
          <div class="form-group col-md-6">
            <div class="form-group">
              ...
              <div class="input-group">
                <input type="number" value="{{  $modelStone->weight  }}" class="form-control calculate-stones" id="1" name="stone_weight[]"
                 data-calculateStones-weight placeholder="Тегло:">
                <span class="input-group-addon">гр</span>
              </div>
            </div>
          </div>
          ...
          <div class="form-group col-md-6">
            <div class="checkbox checkbox-circle checkbox-info peers ai-c mB-15 stone-flow-holder"><input type="checkbox" id="inputCall1"
               name="stone_flow[]" class="peer stone-flow calculate-stones" @if($modelStone->flow == 'yes') checked @endif><label
               for="inputCall1" class="peers peer-greed js-sb ai-c"><span class="peer peer-greed">За леене</span></label>
                        ...
                        <select id="model-stone" name="stones[]" class="form-control" data-calculatePrice-stone>
                            ...
                            @foreach($stones as $stone)
                                <option value="{{ $stone->id }}" @if($modelStone->stone->id == $stone->id) selected @endif data-stone-type="{{ $stone->type }}" data-stone-price="{{ $stone->price }}">
                                    ...
                                    ({{ $modelStone->stone->contour->name }}, {{ $modelStone->stone->size->name }}, {{ $modelStone->stone->style->name }})
                                </option>
                            @endforeach
                        </select>
        ...
        <div class="form-group col-md-6">
          ...
          <div class="input-group">
            <input type="number" class="form-control worksmanship_price" value="{{ $model->workmanship }}" name="workmanship"
             data-calculatePrice-worksmanship>
