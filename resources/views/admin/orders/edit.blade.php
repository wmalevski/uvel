...
foreach($materials as $material){
	if($material->pricesBuy->first() && $material->pricesSell->first()){
		$givenMaterialRowTpl .= '<option value="'. $material->id .'" data-carat="'. $material->carat  .'" data-material="'. $material->id  .'" data-pricebuy="'. $material->pricesBuy->first()->price  .'">'.$material->parent->name.' '.$material->code.', '.$material->code.'</option>';
	}
}
		...
		<div class="modal-body">
			...
			<div class="form-row">
				<div class="checkbox checkbox-circle checkbox-info peers ai-c mB-15">
					<input id="weightWithStones_edit" class="peer" type="checkbox" name="with_stones" data-calculatePrice-withStones @if($order->weight_without_stones == 'no') checked @endif />
			...
			<div class="form-row">
				<div class="form-group col-md-12">
					...
					<div class="input-group">
						<input id="calculate_product" class="form-control" type="text" data-url="ajax/orders/getProductInfo/" name="product_id" placeholder="Сканирай продукт:" />
					</div>
				</div>
			...
			<div class="form-row">
				<div class="form-group col-md-6">
					...
					<select data-url="ajax/orders/getModelInfo/" id="model_id" name="model_id" class="model-select form-control model-filled" data-calculatePrice-model data-search="/ajax/select_search/models/">
						<option value="">Избери</option>
			...
			<div class="form-row model_materials">
				<div class="form-group col-md-12">
					...
					<select name="material_id" class="material_type form-control calculate" data-calculatePrice-material data-search="/ajax/select_search/global/materials/">
						...
						<option value="{{ $material->id }}" data-material="{{ $material->id }}" data-pricebuy="{{ $material->pricesBuy->first()->price }}" @if($material->id == $order->material_id) selected @endif>{{$material->parent->name}} {{$material->code}}, {{$material->color}}</option>
						@endif
						@endforeach
					</select>
				</div>
				<div class="form-group col-md-6">
					...
					<select class="form-control calculate prices-filled retail-price retail_prices" name="retail_price_id" data-calculatePrice-retail>
						<option value="">Избери</option>
						@foreach($order->material->pricesSell as $price)
						<option value="{{ $price->id }}" data-retail="{{ $price->price }}" data-price="{{ $price->price }}" data-material="{{ $price->material_id }}" @if($price->id == $order->retail_price_id) selected @endif>{{ $price->slug }} - {{ $price->price }}лв.</option>
						@endforeach
					</select>
				</div>
				<div class="form-group col-md-3 weight-holder weight-holder-edit">
					...
					<div class="input-group">
						<input id="weight_edit" class="form-control weight calculate" type="text" value="{{ $order->weight }}" name="weight" data-calculatePrice-netWeight placeholder="Тегло:" min="1" max="10000" />
				...
				@endif
					<div class="form-group col-md-8">
						...
						<select name="stones[]" class="form-control" data-calculatePrice-stone data-search="/ajax/select_search/stones/">
							...
							<option value="{{ $stone->id }}" @if($order_stone->stone_id == $stone->id) selected @endif data-type="{{ $stone->type }}" data-price="{{ $stone->price }}">{{$stone->nomenclature->name}} ({{$stone->contour->name}}, {{$stone->size->name}})</option>
							@endforeach
						</select>
					</div>
					<div class="form-group col-md-3">
					...
					<div class="form-group col-md-6">
						<div class="form-group">
							...
							<div class="input-group">
								<input type="number" value="{{$order_stone->weight}}" class="form-control calculate-stones" id="1" name="stone_weight[]" data-calculateStones-weight placeholder="Тегло:" min="0.1" max="100" disabled />
								<span class="input-group-addon">гр.</span>
							</div>
						</div>
					</div>
					<div class="form-group col-md-6">
						<div class="checkbox checkbox-circle checkbox-info peers ai-c mB-15 stone-flow-holder">
							<input type="checkbox" id="inputCall1" name="stone_flow[]" class="peer stone-flow calculate-stones" @if($order_stone->flow == 'yes') checked @endif />
			...
			<div class="form-row">
				<div class="form-group col-md-4">
					...
					<div class="input-group">
						<input id="grossWeight_edit" class="form-control" type="number" name="gross_weight" value="{{$order->gross_weight}}" data-calculatePrice-grossWeight disabled />
						<span class="input-group-addon">гр.</span>
					</div>
				</div>
				<div class="form-group col-md-4">
					...
					<div class="input-group">
						<input id="workmanship" class="form-control worksmanship_price workmanship" type="number" value="{{$order->workmanship}}" name="workmanship" value="0" data-calculatePrice-worksmanship />
			...
			<div class="form-row">
				<div class="form-group col-md-12">
					...
					<select id="store" name="store_id" class="form-control" data-search="/ajax/select_search/stores/" @if ($disable_store_select) disabled @endif >
						...
						<option value="{{ $store->id }}" @if($store->id == $order->store_id) selected @endif>{{$store->name}}, {{$store->location}}</option>
						@endforeach
					</select>
				</div>
			</div>
			...
			<div class="given-material">
				...
				<div class="form-row">
					...
					<div class="form-group col-md-6">
						...
						<select name="given_material_id[]" data-calculateprice-material class="material_type form-control calculate" data-search="/ajax/select_search/global/materials/">
							<option value="">Избери</option>
							@if($xm->pricesBuy->first())
							<option value="{{$xm->id}}" data-carat="{{$xm->carat}}" data-material="{{$xm->id}}" data-pricebuy="{{$xm->pricesBuy->first()->price}}" selected>{{$xm->parent->name}} {{$xm->code}}, {{$xm->color}}</option>
							@endif
						</select>
					</div>
					<div class="form-group col-md-5">
						...
						<div class="input-group">
							<input type="number" class="form-control mat-quantity" name="mat_quantity[]" value="{{$xMat['weight']}}" placeHolder="0" min="0" />
						</div>
					</div>
					<div class="form-group col-md-1">
						<span class="delete-material remove_field" data-materials-remove><i class="c-brown-500 ti-trash"></i></span>
...
<style type="text/css">
	button#internalPrint{position:absolute;right:40px;top:17px;border:none;background:transparent;}
