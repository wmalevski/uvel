        ...
        if(is_array($items) && !empty($items)){
            foreach($items as $item){
                ...
                if($item->attributes->type == 'model'){
                    $table_items .= '<tr class="modelSize"><td></td><td colspan="3" class="modelSize"><div><label for="modelSize">Размер:</label><textarea name="modelSize['.$item->id.']" id="modelSize"></textarea></div></td><td></td></tr>';
                }
