...
class UserPayment extends Model
{
    public function storePayment(){
        ...
        $payment->shipping_method = session('cart_info.2.shipping_method');
        $payment->payment_method = session('cart_info.2.payment_method');
        $payment->user_id = Auth::user()->getId();
        $payment->price = $subtotal;
        $payment->information = session('cart_info.2.information');

        if(session('cart_info.2.shipping_method') == 'ekont'){
            $payment->ekont_address = session('cart_info.2.ekont_address');
        } else if(session('cart_info.2.shipping_method') == 'store'){
            $payment->store_id = session('cart_info.2.store_id');
        }
